...
if (l >= LVL_USER_EFFECT && l <= LVL_USER_THEME)
{
	...
	{
		_confirm_snd = global.user.theme[global.user.theme_curr].snd.equip;
		// Functions level
		if (l == LVL_USER_FUNC)
		...
		if (l == LVL_USER_FUNC && global.user.func_curr == undefined) || (l == LVL_USER_THEME && o != global.user.theme_curr && global.user.theme_curr == USER_THEME.DFLT)
			_confirm_snd = global.user.theme[global.user.theme_curr].snd.unequip;
	}
	else
		_confirm_snd = global.user.theme[global.user.theme_curr].snd.error;
}
