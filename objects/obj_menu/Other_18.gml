/// @descr Option's value cycle results
...
switch (object_index)
{
	...
	case obj_menu_config:
	{
		...
		else if (l == LVL_VIDEO) // Graphics level
		{
			if (o == 0)
			{
		...
		else if (l == LVL_AUDIO) // Music & Sounds level
		{
			global.config.audio.emitter[o].volume = clamp((round((global.config.audio.emitter[o].volume + (0.1 * _cycle_sign)) * 100) / 100), 0, 1);
			fn_config_file_save();
		}
		else if (l == LVL_ACCESS) // Accessibility level
		{
			if (o == 0)
			{
