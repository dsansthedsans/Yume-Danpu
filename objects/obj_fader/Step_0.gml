...
if (type.fade.act == true)
{
	...
	if (stage == -1 && type.fade.iris.act == false)
		stage = 0;
	else if (stage >= -1 && type.fade.iris.act == true)
	{
		var _iris = type.fade.iris;
		_iris.siner += 0.1;
		_iris.xOffset += (cos(_iris.siner) / 2);
		_iris.yOffset += (sin(_iris.siner) / 2);
		_iris.radius = fn_lerp(_iris.radius, 0, _iris.radiusSpd);
		_iris.alpha = fn_lerp(_iris.alpha, 1, _iris.alphaSpd);
		...
		if (stage == -1 && _iris.radius <= _iris.radiusTgt && _iris.snd_id != undefined)
			stage = 0;
	}
	// Fades out of the source room
	if (stage == 0)
	{
		type.fade.alpha = fn_lerp(type.fade.alpha, 1, type.fade.alphaSpd);
	...
	else if (stage == 1)
	{
		...
		{
			type.fade.alpha = fn_lerp(type.fade.alpha, 0, type.fade.alphaSpd);
