...
if (talk.act == true && talk.stg > -1)
{
	...
	else if (talk.type.gift.act == true)
	{
		...
		if (_gift.content_idx != -1 && _gift.content[_gift.content_idx] != -1 && _gift.content[_gift.content_idx].unlocked == false)
		{
			...
			if (array_contains(global.user.eff, _content) == true)
			{
				
			}
			else if (array_contains(global.user.fcn, _content) == true)
				fn_user_fcn_unlock(array_get_index(global.user.fcn, _content));
			else if (array_contains(global.user.thm, _content) == true)
			{
...
// Talk trigger
if (talkTrig.act == true && fn_config_key_pressed(talkTrig.key) == true && talk.stg == -1 && move.stg == -1)
{
